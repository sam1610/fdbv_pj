...
// const formatAgentId = (phone) => {
//     if (!phone) return null;
//     // Remove all non-numeric characters (removes +, spaces, dashes)
//     const clean = phone.replace(/[^0-9]/g, ''); 
//     // Result: "AGENT_97334667480" (Valid for AWS)
//     return `AGENT_${clean}`; 
// };

// // PART A: Keep the Ref updated whenever location changes
//     useEffect(() => {
//         latestLocationRef.current = agentLocation;
//     }, [agentLocation]);

//     // PART B: The strict interval timer (Independent of movement)
// // ✅ AgentDashboard.js - Tracking Interval
//     useEffect(() => {
//         if (!agentPhone) return;
//         const deviceId = formatAgentId(agentPhone);
...
//                 // ONLY update the Tracker (Cheap & Fast)
//                 await locationClient.send(new BatchUpdateDevicePositionCommand({
...
// AgentDashboard.jsx
// Optimized, cost-aware, smooth-tracking compatible with DeliveryOptimizer
...
// /* ------------------------------------------------------------------
//    CONFIG – aligned with DeliveryOptimizer
// -------------------------------------------------------------------*/
// const TRACKING_INTERVAL_MS = 20000;          // cheapest safe rate
// const MIN_MOVE_METERS = 15;                  // ignore GPS jitter
// const DEFAULT_CENTER = { lat: 26.0935, lng: 50.4880 };
...
//   if (!loc) return null;
//   try {
//     const d = typeof loc === 'string' ? JSON.parse(loc) : loc;
//     const lat = parseFloat(d.lat ?? d.latitude ?? d?.latitude?.N);
//     const lng = parseFloat(d.lng ?? d.longitude ?? d?.longitude?.N);
...
// const agentDeviceId = (phone) => `AGENT_${phone.replace(/\D/g, '')}`;

// /* ------------------------------------------------------------------
//    COMPONENT
// -------------------------------------------------------------------*/
// export default function AgentDashboard({ agentPhone, businessLocation }) {
...
//     (async () => {
//       const center = parseCoords(businessLocation) || DEFAULT_CENTER;
//       const map = await createMap({
...
//   /* --------------------------------------------------------------
//      SMOOTH AGENT MARKER (NO AWS COST)
//   ---------------------------------------------------------------*/
//   useEffect(() => {
//     if (!mapReady || !agentLoc) return;
...
//     const start = agentMarker.current.getLngLat();
//     const end = { lng: agentLoc.lng, lat: agentLoc.lat };
//     let t = 0;
...
//       agentMarker.current.setLngLat([lng, lat]);
...
//   /* --------------------------------------------------------------
//      CHEAP TRACKER UPDATES (DeliveryOptimizer compatible)
//   ---------------------------------------------------------------*/
//   useEffect(() => {
//     if (!agentPhone) return;
//     const deviceId = agentDeviceId(agentPhone);

//     const timer = setInterval(async () => {
//       if (!agentLoc) return;
//       if (lastSentRef.current && haversine(lastSentRef.current, agentLoc) < MIN_MOVE_METERS) return;
...
//           Updates: [{
//             DeviceId: deviceId,
//             Position: [agentLoc.lng, agentLoc.lat],
//             SampleTime: new Date(),
//           }],
//         }));

//         lastSentRef.current = agentLoc;
//       } catch (e) {
//         console.warn('Tracker update skipped', e);
//       }
...
//   /* --------------------------------------------------------------
//      DATA (Orders) – simplified
//   ---------------------------------------------------------------*/
//   useEffect(() => {
//     if (!agentPhone) return;

//     client.models.BusinessData.ByAgent({
//       gsi1pk: `AGENT#${agentPhone}`,
//       sk: { beginsWith: 'ORDER#' },
//       limit: 50,
//     }).then(({ data }) => {
//       setOrders(data.filter(o => ['DELIVERING', 'DELIVERED'].includes(o.orderStatus)));
//     });
//   }, [agentPhone]);
...
//       {/* Minimal HUD */}
//       <div className="absolute top-3 left-3 z-50 bg-slate-800/90 text-white px-4 py-2 rounded-xl shadow">
//         <div className="font-bold">Agent Tracking</div>
//         <div className="text-xs text-green-400">● Live (Low‑Cost)</div>
//         <div className="text-[10px] text-slate-400">Orders: {orders.length}</div>
//       </div>
//     </div>
//   );
// }
...
//    CONFIG – Aligned with DeliveryOptimizer
// -------------------------------------------------------------------*/
// const TRACKING_INTERVAL_MS = 20000;          // 20s = Cheap & Safe Buffer
// const MIN_MOVE_METERS = 15;                  // Ignore GPS jitter < 15m
// const DEFAULT_CENTER = { lat: 26.0935, lng: 50.4880 };

// /* ------------------------------------------------------------------
//    HELPERS
// -------------------------------------------------------------------*/
// // Standard ID format for AWS Location Service
// const agentDeviceId = (phone) => `AGENT_${phone.replace(/\D/g, '')}`;

// // Haversine Distance (km) for Order Logic
// const calculateDistance = (lat1, lon1, lat2, lon2) => {
//     if (!lat1 || !lon1 || !lat2 || !lon2) return null;
//     const R = 6371; 
//     const dLat = (lat2 - lat1) * Math.PI / 180;
...
// // Haversine Distance (meters) for GPS Jitter Logic
...
//     const markersRef = useRef({});            
//     const restaurantMarkersRef = useRef({});  
//     const agentMarkerRef = useRef(null); // The blue dot on the driver's map
//     const lastSentRef = useRef(null);    // For jitter check
...
//     useEffect(() => {
//         if (mapInstance.current || !mapContainerRef.current) return;
//         const startLoc = getCoordinates(businessLocation) || DEFAULT_CENTER;
...
//                 // Native Geolocate Control (Source of Truth)
//                 const geolocate = new maplibregl.GeolocateControl({
//                     positionOptions: { enableHighAccuracy: true },
//                     trackUserLocation: true,
//                     showUserLocation: false // We draw our own smooth marker instead
//                 });
...
//     // ------------------------------------------------------------------
//     // 2. SMOOTH LOCAL MARKER (Client-Side Animation)
//     // ------------------------------------------------------------------
//     useEffect(() => {
//         if (!isMapReady || !agentLocation) return;
//         const map = mapInstance.current;

//         // If no marker yet, create one immediately
//         if (!agentMarkerRef.current) {
//             const el = document.createElement('div');
//             el.className = 'driver-marker';
//             el.innerHTML = '<div style="width:16px;height:16px;background:#3b82f6;border:2px solid white;border-radius:50%;box-shadow:0 0 10px #3b82f6;"></div>';
...
//     // ------------------------------------------------------------------
//     // 3. AWS TRACKER UPDATE (Cheap & Robust)
//     // ------------------------------------------------------------------
//     useEffect(() => {
//         if (!agentPhone) return;
//         const deviceId = agentDeviceId(agentPhone);
...
//             // Jitter Filter: Don't send if moved < 15 meters
//             if (lastSentRef.current && haversineMeters(lastSentRef.current, agentLocation) < MIN_MOVE_METERS) return;
...
//                     Updates: [{
//                         DeviceId: deviceId,
//                         Position: [agentLocation.lng, agentLocation.lat],
//                         SampleTime: new Date(),
//                     }],
//                 }));

//                 console.log(`📡 Tracker Sent: ${agentLocation.lat}, ${agentLocation.lng}`);
//                 lastSentRef.current = agentLocation; // Update last sent
//             } catch (e) {
//                 console.warn('Tracker update skipped', e);
//             }
//         }, TRACKING_INTERVAL_MS);
...
//         return () => clearInterval(timer);
//     }, [agentLocation, agentPhone]);


//     // ------------------------------------------------------------------
//     // 4. DATA LOGIC (Orders, Reset, Filter) - Kept from your code
//     // ------------------------------------------------------------------
...
//     // Grid List Logic
//     const gridList = useMemo(() => {
//         let origin = agentLocation || agentProfileLoc || DEFAULT_CENTER;
//         if (isNaN(Number(origin.lat))) origin = DEFAULT_CENTER;
...
//                              <h2 className="text-white font-bold text-lg">Daily Manifest</h2>
//                              <div className="flex items-center gap-2">
//                                 <button onClick={handleReset} className="bg-red-900/50 hover:bg-red-800 text-red-300 text-xs font-bold px-3 py-1.5 rounded border border-red-800 flex items-center gap-1 transition active:scale-95">↻ RESET (24h)</button>
//                                 <div className="text-xs text-slate-400">{gridList.length} Orders</div>
//                              </div>
//                          </div>
//                     </div>
...
//                         <div className="pt-4 pb-8 flex justify-center">
//                             {(nextToken || (!isHistoryMode && rawOrders.length > gridList.length)) && (
//                                 <button onClick={handleLoadMore} disabled={isLoadingMore} className="bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold py-3 px-6 rounded-full border border-slate-600 text-sm flex items-center gap-2 transition-all active:scale-95 shadow-lg">
//                                     {isLoadingMore ? "Loading..." : "👇 MORE HISTORY (+10)"}
//                                 </button>
//                             )}
//                             {isHistoryMode && !nextToken && gridList.length > 0 && <div className="text-slate-500 text-xs italic">All history loaded.</div>}
//                         </div>
//                     </div>
//                 </div>
//             </div>
...
//     // ------------------------------------------------------------
//     // 🚨 KEY FIX: NORMALIZE IDS
//     // ------------------------------------------------------------
//     // 1. DB ID: Always Ensure "AGENT#+" + number
//     const dbAgentPK = useMemo(() => {
//         if (!agentPhone) return null;
//         // Strip everything except numbers, then add AGENT#+
//         const clean = agentPhone.replace(/[^0-9]/g, ''); 
//         return `AGENT#+${clean}`;
//     }, [agentPhone]);

//     // 2. TRACKER ID: Always "AGENT_" + number (AWS Trackers dislike +)
...
//     // 1. HELPER: Load Restaurant Configs
//     const loadRestaurantConfigs = useCallback(async (pks) => {
//         const missingPks = pks.filter(pk => !restaurants[pk]);
//         if (missingPks.length === 0) return;
...
//     // 4. AWS TRACKER (Using normalized trackerDeviceId)
//     useEffect(() => {
...
//                     TrackerName: outputs.custom.amazon_location_service.trackers.default,
//                     Updates: [{
//                         DeviceId: trackerDeviceId, // Uses "AGENT_973..." (No +)
//                         Position: [agentLocation.lng, agentLocation.lat],
//                         SampleTime: new Date(),
//                     }],
//                 }));
//                 console.log(`📡 Tracker Sent: ${trackerDeviceId}`);
//                 lastSentRef.current = agentLocation; 
//             } catch (e) { console.warn('Tracker update skipped', e); }
//         }, TRACKING_INTERVAL_MS);
//         return () => clearInterval(timer);
//     }, [agentLocation, trackerDeviceId]);

//     // ------------------------------------------------------------------
//     // 🚀 5. OBSERVE QUERY (Using normalized dbAgentPK)
//     // ------------------------------------------------------------------
//     useEffect(() => {
//         if (!dbAgentPK) return;

//         console.log(`🔍 Observing Orders for: ${dbAgentPK}`); // Debug Log

//         // ObserveQuery: Automatically fetches list AND subscribes to updates
//         const sub = client.models.BusinessData.observeQuery({
//             filter: {
//                 gsi1pk: { eq: dbAgentPK }, // Uses "AGENT#+973..." (With +)
//                 sk: { beginsWith: 'ORDER#' }
//             }
//         }).subscribe({
//             next: ({ items }) => {
//                 // Filter locally for status (active deliveries)
//                 const validOrders = items.filter(o => ['DELIVERING', 'DELIVERED'].includes(o.orderStatus));
                
//                 // Sort Descending (Newest first)
//                 validOrders.sort((a, b) => new Date(b.createdAt || b.orderDate) - new Date(a.createdAt || a.orderDate));

//                 setRawOrders(validOrders);

//                 // Load Configs for any new restaurants found
//                 const initialPks = [...new Set(validOrders.map(o => o.pk))];
//                 loadRestaurantConfigs(initialPks);
//             },
...
//     }, [dbAgentPK, loadRestaurantConfigs]);

//     // ------------------------------------------------------------------
//     // 6. RENDER ORDERS ON MAP
//     // ------------------------------------------------------------------
//     useEffect(() => {
//         if (!isMapReady || !mapInstance.current) return;
//         const map = mapInstance.current;
...
//         // Apply filters for Map visibility
//         const visibleMapOrders = rawOrders.filter(o => {
//             if (o.orderStatus === 'DELIVERED' && !showDelivered) return false;
            
//             // Time Filter: If showHistory is false, only show last 24h
//             if (!showHistory) {
//                 const t = o.createdAt ? new Date(o.createdAt).getTime() : now;
//                 return t > twentyFourHoursAgo;
//             }
...
//         visibleMapOrders.forEach(order => {
//             const config = restaurants[order.pk];
//             const loc = getCoordinates(order.pickupLocation);
//             if (!uniqueRestaurants.has(order.pk)) {
//                 if (config) uniqueRestaurants.set(order.pk, config);
...
//             el.style.transition = 'all 0.3s ease';
//             el.style.backgroundColor = isDelivered ? '#22c55e' : '#fbbf24';
//             if (isRelated) { el.style.border = `3px solid ${restColor}`; el.style.transform = 'scale(1.3)'; el.style.zIndex = '50'; } 
//             else { el.style.border = '3px solid white'; el.style.transform = 'scale(1)'; }
//             el.style.boxShadow = '0 2px 4px rgba(0,0,0,0.3)';
//             const marker = new maplibregl.Marker({ element: el }).setLngLat([coords.lng, coords.lat]).addTo(map);
//             el.addEventListener('click', (e) => { e.stopPropagation(); setSelectedOrder(order); setSelectedRestaurant(null); map.flyTo({ center: [coords.lng, coords.lat], zoom: 15 }); });
//             markersRef.current[order.sk] = marker;
//         });

//     }, [rawOrders, focusedRestaurant, isMapReady, restaurants, showDelivered, showHistory]); 
...
//         // Apply same filtering as map (History + Delivered)
//         const now = Date.now();
//         const twentyFourHoursAgo = now - (24 * 60 * 60 * 1000);
...
//             return { ...order, _dist: dbDist, _time: dbTime, _sortDist: sortDist }; 
//         }).sort((a,b) => parseFloat(a._sortDist) - parseFloat(b._sortDist));
//     }, [rawOrders, agentLocation, agentProfileLoc, showHistory, showDelivered]);

//     const markAsDelivered = async (order) => {
//         try {
//             await client.models.BusinessData.update({ pk: order.pk, sk: order.sk, orderStatus: 'DELIVERED', deliveryAgentId: dbAgentPK });
            
//             // For profile updates, we check if the profile exists first or just update blindly
//             // Assuming business logic: Update Agent Location
//             // Ensure we use the formatted ID
//             if (dbAgentPK) {
...
//                              <h2 className="text-white font-bold text-lg">Daily Manifest</h2>
//                              <div className="flex items-center gap-2">
//                                 <button onClick={() => { setShowHistory(false); }} className="bg-red-900/50 hover:bg-red-800 text-red-300 text-xs font-bold px-3 py-1.5 rounded border border-red-800 flex items-center gap-1 transition active:scale-95">↻ RESET (24h)</button>
//                                 <div className="text-xs text-slate-400">{gridList.length} Orders</div>
//                              </div>
//                          </div>
//                     </div>
//                     <div className="flex-1 overflow-y-auto p-4 space-y-3 pb-20">
//                         {gridList.length === 0 && <div className="text-center text-slate-500 mt-10">No recent orders found.</div>}
...
//                         <div className="pt-4 pb-8 flex justify-center">
//                             {!showHistory && rawOrders.length > gridList.length && (
//                                 <button onClick={() => setShowHistory(true)} className="bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold py-3 px-6 rounded-full border border-slate-600 text-sm flex items-center gap-2 transition-all active:scale-95 shadow-lg">
//                                     👇 SHOW OLDER ORDERS
//                                 </button>
//                             )}
//                             {showHistory && <div className="text-slate-500 text-xs italic">All history shown.</div>}
//                         </div>
//                     </div>
//                 </div>
//             </div>
...
const AgentDashboard = ({ agentPhone, businessLocation }) => {
    ...
    useEffect(() => {
        ...
        const timer = setInterval(async () => {
            if (!agentLocation) return;
            // Don't send if we haven't moved enough
            if (lastSentRef.current && haversineMeters(lastSentRef.current, agentLocation) < MIN_MOVE_METERS) return;
