            ...
            try {
                ...
                if (config) {
                    ...
                    try {
                        ...
                        if(loc && loc.latitude) setCoords({ lat: String(loc.latitude), lng: String(loc.longitude) });
                    } catch(e) {}
                }
    ...
    const handleRequestPhoneVerification = async () => {
        ...
        try {
            const { data: response } = await client.mutations.registerPhoneNumber({
                ...
                businessPhone: phoneNbr, // ✅ Use Prop directly
                businessPhoneOwner: phoneNbr,
                verificationMethod: verificationMethod // ✅ Pass SMS vs VOICE
            });
        ...
        <div className="space-y-8 max-w-sm mx-auto pt-4">
            ...
            <div className="bg-slate-800/40 p-6 rounded-xl border border-slate-700">
                ...
                <div className="space-y-4">
                    ...
                    {/* Location UI simplified for brevity but logic is preserved above */}
                    <button 
...
// =========================================================
// 4️⃣ SUB-COMPONENT: ItemsSection (Unchanged)
// =========================================================
const ItemsSection = ({ pk, onUpdate }) => {
    const BASE64_HEADER = "data:image/jpeg;base64,";
    const [item, setItem] = useState({ name: '', price: '', description: '', quantity: '', stockStatus: true, imageUrl: '' });
    ...
    const selectItemToEdit = (selectedItem) => {
        ...
        setItem({
            name: selectedItem.name, price: selectedItem.unitPrice.toString(), description: selectedItem.description || '',
            quantity: selectedItem.quantity || '', stockStatus: selectedItem.stockStatus, imageUrl: previewImage 
        });
        setCategory(selectedItem.itemCategory || ''); setShowItemSuggestions(false);
    };
    ...
    const submit = async () => {
        ...
        const payload = {
            pk, name: item.name.trim(), unitPrice: parseFloat(item.price), itemCategory: category.trim().toUpperCase(),
            description: item.description, stockStatus: item.stockStatus, quantity: item.quantity ? parseInt(item.quantity) : 0, imageUrl: dbImageString
        };

        try {
            if (editingId) { await client.models.BusinessData.update({ ...payload, sk: editingId }); } 
            else {
            ...
            await fetchItems(); if (onUpdate) onUpdate(); setEditingId(null);
            setItem({ name: '', price: '', description: '', quantity: '', stockStatus: true, imageUrl: '' }); setCategory('');
    ...
    return (
        <div className="space-y-4 max-w-sm mx-auto pt-4 animate-fade-in pb-20">
            ...
            <button onClick={submit} disabled={saving} className="w-full py-3 rounded-xl font-black text-white bg-indigo-600 hover:bg-indigo-500 shadow-lg">{saving ? "SAVING..." : (editingId ? "UPDATE ITEM" : "ADD NEW ITEM")}</button>
        </div>
    );
};
