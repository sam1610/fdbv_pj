...
const DashboardView = ({ phoneNbr, filterDays = 1, setModal }) => {
   ...
   useEffect(() => {
        // 🚨 1. LOUD DEBUGGING: Does the effect even trigger?
        console.log(`🚨 AI Effect Triggered! Phone: ${phoneNbr} | isMarketingOpen: ${isMarketingOpen}`);
        
        if (!phoneNbr || !isMarketingOpen) {
            console.log("🛑 Stopping early: Phone is missing OR Accordion is closed.");
            return;
        }

        const fetchAiCache = async () => {
            console.log("🚀 fetchAiCache function is starting...");
            try {
                const formattedPhone = phoneNbr.startsWith('+') ? phoneNbr : `+${phoneNbr}`;
                console.log(`🔍 Querying DynamoDB for PK: BUSINESS#${formattedPhone} | SK: CACHE#AI_RECOMMENDATIONS`);
                
                // 1. Fetch the cached list
                const { data: cacheData, errors } = await client.models.BusinessData.get({
                    pk: `BUSINESS#${formattedPhone}`,
                    sk: 'CACHE#AI_RECOMMENDATIONS'
                });

                if (errors) console.error("❌ GraphQL Errors fetching cache:", errors);
                console.log("📦 Raw Data from DynamoDB:", cacheData);
                ...
                if (cacheData && cacheData.recommendations) {
                    ...
                    console.log("🤖 Parsed AI Recommendations List:", cachedList);
                    ...
                    console.log("✅ Eligible Live Phones (No active offers):", eligiblePhones);

                    const finalDisplayList = cachedList.filter(rec => eligiblePhones.includes(rec.phone));
                    console.log("🎯 Final Matched List for Dashboard:", finalDisplayList);

                    setAiRecommendations(finalDisplayList);
                } else {
                    console.log("⚠️ Cache data was empty or missing 'recommendations' attribute.");
                    setAiRecommendations([]);
                }
            } catch (e) {
                console.error("🔥 FATAL ERROR loading AI recommendations:", e);
            }
        };
