...
//     if (typeof order.phone === 'string' && order.phone.trim() !== '') return order.phone;
//     if (order.phone && order.phone.S) return order.phone.S;
//     if (typeof order.customer === 'string' && order.customer !== 'Unknown') return order.customer;
//     if (order.gsi2pk && typeof order.gsi2pk === 'string') {
//         const parts = order.gsi2pk.split('#');
...
//               const strictAgentId = agentObj ? (agentObj.id || agentObj.sk) : null;
...
//                   const custPhone = formatPhone(extractPhone(order));
...
//           const agentObj = findAgent(rawAgentId);
//           const finalAgentId = agentObj ? (agentObj.id || agentObj.sk) : rawAgentId; 
...
//                               <div className="text-[10px] text-slate-300 mb-2 font-mono">Phone: {formatPhone(extractPhone(o))}</div>
...
const extractPhone = (order) => {
    if (!order) return "Unknown";
    if (order.phone) {
        return typeof order.phone === 'string' ? order.phone : (order.phone.S || "Unknown");
    }
    if (order.customer && order.customer !== 'Unknown') return order.customer;
...
const DeliveryOptimizer = ({
  ...
  const fetchOrderItems = async (order) => {
      if (orderItemsCache[order.sk]) return orderItemsCache[order.sk];
      try {
          const phoneNbrPk = order.pk.split('#')[1];
          const orderIdPart = order.sk.split('#')[1];
          const { data: lineItems } = await client.models.BusinessData.listByBusiness({
        ...
        <div className={`absolute top-0 bottom-0 left-0 w-80 bg-slate-900 border-r border-slate-700 shadow-2xl transform transition-transform duration-300 flex flex-col ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              ...
              {displayedOrders.map((o, i) => {
                  ...
                  const custPhone = formatPhone(extractPhone(o));
                  const agentPhoneStr = formatPhone(agentObj?.phone || getCleanPhone(assignedAgentId));

                  // ✅ COMPLETELY IMMUTABLE DELIVERED CARD UI
                  if (isDelivered) {
                      return (
                          <div 
                              key={o.sk || i} 
                          ...
                          >
                              ...
                              <div className="mb-2 space-y-1">
                                  ...
                                  <p className="text-[10px] text-slate-400 font-mono">Cust: {custPhone}</p>
                  ...
                  >
                      ...
                      <div className="mb-2">
                            <p className="font-bold text-xs truncate text-slate-200">{custPhone}</p>
