...
//     console.log("DeliveryOptimizer Orders :", {orders } );
...
//   // --- 2. Watch for Assignment Changes & Re-plot ---
//   useEffect(() => {
//     if (mapInstance.current) {
//         plotOrdersOnMap(orders, mapInstance.current, assignments);
//     }
//   }, [assignments, orders]); // This triggers whenever assignments update!


//   // --- 3. Plot Orders Function (With Agent Name Logic) ---
...
//                     ORDER ${index + 1} 
//                     ${agentName ? `<span style="color: #8B4513; font-size: 14px;"> (${agentName})</span>` : ''}
//                   </div>
...
//       // This state update triggers the useEffect above to re-plot markers
//       setAssignments(newAssignments);
//       alert("Routes optimized & assigned!");
...
//              <span className="w-2 h-2 bg-blue-500 rounded-full mr-2"></span> Order
//            </div>
//         </div>
//       </div>

//       {/* 2. SIDEBAR PANEL (Right) */}
...
//               disabled={loading}
//               className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-lg shadow-lg transition-all h-12 w-12 md:w-full md:h-auto md:py-3 flex items-center justify-center"
...
//                    <span className="hidden md:inline ml-2">Run Auto-Assign</span>
//                  </>
//               )}
//           </button>
//         </div>
...
//                     className="bg-slate-800 rounded-lg border border-slate-700 cursor-pointer hover:bg-slate-700 hover:border-blue-400 transition-all flex items-center group justify-center md:justify-between p-2 md:p-4"
//                  >
//                     <span className="bg-blue-600 text-white text-sm font-bold w-8 h-8 md:w-6 md:h-6 rounded-full flex items-center justify-center shrink-0 shadow-sm">
//                       {i+1}
//                     </span>

//                     <div className="hidden md:block flex-1 ml-3 min-w-0">
//                       <p className="font-bold text-sm truncate text-left">
//                         {o.customer || 'Unknown Phone'}
//                       </p>
//                       {/* Show assigned agent in list too if available */}
//                       {assignments[o.sk] && (
//                          <p className="text-xs text-yellow-500 text-left font-mono">
//                            → {agents.find(a => a.id === assignments[o.sk])?.name || 'Assigned'}
//                          </p>
//                       )}
//                       <p className="text-xs text-slate-400 text-left truncate">
//                         {o.sk.split('#')[1]}
//                       </p>
//                     </div>
                    
//                     <span className="hidden md:block text-slate-500 group-hover:text-white ml-2">📍</span>
//                  </div>
//              ))}
//         </div>
//       </div>
...
// import 'maplibre-gl-js-amplify/dist/public/amplify-map.css';
// import { client } from '../DataHook/amplifyClient'; 
// // ✅ Import your custom update utility
...
//           ],
//           zoom: 12,
//           attributionControl: false // Optional: Clean up UI
//         });
...
//   // --- Watch for Assignment Changes & Re-plot Tooltips ---
//   useEffect(() => {
//     if (mapInstance.current) {
//         plotOrdersOnMap(orders, mapInstance.current, assignments);
//     }
...
//   // --- Plot Orders (With Dropdown Sync) ---
//   const plotOrdersOnMap = (ordersToPlot, map, currentAssignments) => {
//       if (!map) return;
...
//   // --- Optimization Logic (Round Robin) ---
//   const runOptimization = async () => {
//     setLoading(true);
//     try {
//       const response = await client.graphql({
...
//   // --- ✅ CONFIRM DISPATCH (Uses updateRec) ---
//   const handleDispatch = async () => {
//     if (Object.keys(assignments).length === 0) return;
//     setSaving(true);

//     try {
//       // 1. Group orders by Agent ID to minimize API calls
//       // Structure: { "AGENT#123": ["ORDER#1", "ORDER#2"], "AGENT#456": ["ORDER#3"] }
...
//       const pk = orders[0]?.pk; // All orders belong to same Business
//       if (!pk) throw new Error("Missing PK");

//       // 2. Create a Promise for each Agent group
//       const updatePromises = Object.entries(updatesByAgent).map(([agentId, skList]) => {
//           return updateRec(pk, skList, {
//               gsi1pk: agentId,          // ✅ Put AGENT#Phone into gsi1pk
//               deliveryAgentId: agentId, // Optional: Update dedicated field if exists
...
//                  <div 
//                     key={o.sk} 
//                     onClick={() => handleOrderClick(o.sk)} // Clicking row flies to map
...
//                         // Stop propagation so clicking dropdown doesn't fly map
